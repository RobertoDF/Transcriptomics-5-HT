...
results = {"Htrs transcription overview":
               "We analysed the single-cell scRNA-seq dataset provided by the Allen Institute {Yao, 2023 #2828} "
                "focusing on the transcription of Htrs RNA across approximately 4 million brain cells. The scRNA-seq dataset comprehensively encompassed all known 14 Htr subtypes. "
               f"{round(((exp>threshold_expression).sum(axis=1).astype(bool).sum()/exp.shape[0])*100,2)}% of cells transcribed RNA of at least one Htr. "
               ...
               f"at least 1 every 5 cells. Average amount of RNA transcription also varied across receptors (Supplementary Figure 1A). "
               ...
               f"to be transcribed more at the single cell level. "
               f"Beside the prevalence and amount of transcription, also the distributionacross the brain was considerably different. This is exemplified "
               ...
               f"Pattern of Htrs transcription across different neurotransmitter groups exhibited a relatevely high Pearson correlation coefficient "
               f"(r={round(corr_by_neurotransmitter.stack().mean(),2)}±{round(corr_by_neurotransmitter.stack().sem(),2)}). "
               f"Sero and cholinergic neurons (Chol) showed the most distinct patterns of transcription with respectively r={round(corr_by_neurotransmitter.mean()['Sero'],2)}±"
               ...
               f"To better evaluate the uniqueness of Htrs RNA transcription per group we employed a Random Forest "
               f"Classifier aiming at decoding the grouping variable from the Htrs transcription. "
               ...
               f"Chol={round(cm_neurotransmitter['Chol']['Chol'], 2)}%). Cells not transcribing any neurotranmitter were also "
               ...
               f"is determined mainly by transcription of Htr1a and Chol neurons by Htr4 and Htr5b. Crucially, absence of transcription can also contribute to the classification, e.g., "
               f"cells not transcribing any neurotransmitter are identified mainly by absence of any Htr. "
               ...
               f"Across classes, differences in Htrs transcription were more striking (Figure 1D). {report_class.loc['recall'][report_class.loc['recall']>.4].shape[0]} groups could be identified "
               f"with a TP rate >40%: {formatted_class_string} (Supplementary Figure 3B). 04 DG-IMN Glut were charachterized by high transcription of Htr4 and absence of "
               ...
               f"were identified by absence of any Htr transcription. The use of only Htrs transcription reached a unexpected {round(accuracy_class,2)}% accuracy in decoding classes. "
               f"Correlation between Htrs transcription across the totality of cells ranged from {corr_by_cell.min().values[0]} ({'-'.join(corr_by_cell.idxmin())}) to {corr_by_cell.max().values[0]} "
               ...
               "authors to determine neurotransmitter transcription {Yao, 2023 #2828}. "
               f"Across the entire dataset we observed that the most transcribed genes, Htr1f and Htr2c, were regurarly transcribed whenever the RNA of any other Htr was detected (Figure 1F). "
               ...
               f"by differential prevalence of Htrs in each neighborhoods. "
               f"{round(at_least_2_receptors.mean().values[0], 2)}±{round(at_least_2_receptors.sem().values[0], 2)}% "
               f"of Htr-transcribing cells exhibits at least "
               f"2 Htrs (Figure 1G), therefore, only in a minority of cases a cell was found to transcribe uniquely one Htr "
               f"({round(((joined_boolean['Number of Htrs'].value_counts() / joined_boolean.shape[0]) * 100)[1], 2)}% of the totality of cells). "
               f"Surprisingly, {round(at_least_5_receptors.mean().values[0], 2)}±{round(at_least_5_receptors.sem().values[0], 2)}% of Htr-transcribing cells transcribed at least 5 Htrs. "
               f"The extensive transcription of different Htr families within the same cell point at the complexity of the 5-HT system even "
           ...
           "Htr1 & Htr5": u"Receptors belonging to these two families have an inhibitory effect on the host cell, they are coupled to G\u1D62 and cause a downstream decrease of cAMP "
                          ...
                          "transcription across classes was highly correlated between the RNA-seq and MERFISH datasets (Figure 2A) and show an almost perfect proportional relationship. "
                          "Highest transcription was found in 5-HT neurons of the mid- and hindbrain (class 22 MB-HB Sero, Figure 2C), nonetheless, cortical excitatory neurons (01 IT-ET Glut), "
                          "like in the majority of Htrs, "
                          "contained the higher absolute number of cells transcribing Ht1a. Subclasses located in the hippocampus (HPF) "
                          "contained most of the cells transcribing Htr1a (see online visualizer, 'Overview genes by class'). "
                          ...
                          f"Only {perc_enriched_htr1a}% of Htr1a transcribing cells were contained in enriched clusters, pointing at a relatively low importance in the clustering algorithm. "
                          "Looking at the spatial distribution across divisions, the highest prevalence was found in the pallidum (PAL) and HPF, two telencephalic regions (Figure 2D). "
                          ...
                          "reflection of the high prevalence in Sero neurons outlined above, "
                          ...
                          "two structures linked to generation of theta waves {Winson, 1978 #2908} and contaiing Chol neurons, contributed substantially to the transcription in PAL. Notably, "
                          ...
                          "Htr1b exhibited a more diverse pattern of transcription across neighboroods (Figure 3A) ranging from 10 to 30%. "
                          ...
                          "Colocalization showed a similar pattern compared to Htr1a (Figure 3B) and also here only a minority of cells transcribed Htr1b alone (<10%). "
                          ...
                          f"Also in this case, 01 IT-ET Glut exhibited the highest absolute number of Htr1b expessing cells, specifically cells in the nucleus of the lateral olfactory tract "
                          f"(NLOT, see online visualizer, 'Overview genes by class'). "
                          ...
                          f"Htr1d was transcribed at a much lower level, never exceeding 7% prevalence in any neighborhood (Supplementary Figure 5A). "
                          f"It colocalized at highest levels with "
                          f"Htr2c and Htr1f (Supplementary Figure 5B) and only rarely was transcribed alone (<5%). "
                          f"Similarly to Htr1b, transcription was highest in 09 CNU-LGE GABA and 22 MB-HB Sero (Supplementary Figure 5C). "
                          ...
                          f"The paraventricular nucleus of the thalamus (PT and PVT) showed the highest prevalence at only >4% (Supplementary Figure 5D-E-F). \n"
                         
                          f"Htr1f, surprisingly, showed the highest levels of transcription of all Htr in the RNA-seq dataset. Higher prevalence was found in the Pallium and Subpallium groups (Figure 4A), "
                          ...
                          f"decreased linearly with the number of cotranscribed Htrs (Figure 4B). "
                          f"Notably, the slope of the linear regression between values provided by RNA-seq and MERFISH was "
                          f"significantly lower pointing at a difference absolute prevalence prer class (Figure 4C). The two datasets are, however, still highly correlated, with 66% of shared variability. "
                          f"This was the case also for other Htr2a, Htr2c and Htr4. Htr1f was broadly transcribed across almost all classes, "
                          f"including some non-neuronal cells, pineal gland cells were a notable exception. "
                          f"In absolute numbers, cortical glutamatergic cells showed the highest transcription. Various subclasses located in L5, claustrum (CLA) and HPF exhibited prevalence >50% "
                          f"(see online visualizer, 'Overview genes by class'). "
                          f"Spatial distribution showed a peculiarly asymettric pattern with transcription concentrated in the most anterior regions. "
                          ...
                          f"Interestingly, in the CTXsp, transcription in Glut and Gaba neurons was anticorrelated. High prevalence in GLut neurons corresponded to lower prevalence in Gaba "
                          f"and vice versa. In CLA and EPd HTR1f was transcribed mainly in Glut neurons, while in the amygdala (LA, BLA, BMA) in GAba neurons predominantly. "
                          f"Isocortex and HPF "
                          ...
                          "was previously observed using immunohistochemistry {Bruinvels, 1994 #2929}. The broad transcription of Htr1f observed in the RNA-seq dataset "
                          ...
                          f"Htr5a was transcribed at 8-16% prevalence across all neighborhoods (Supplementary Figure 6A) "
                          f"and colocalized the most with Htr1f, Htr2c and Htr2a (Supplementary Figure 6B). transcription was broadly distributed across many classes, "
                          ...
                          f""
                          f"Htr5b was transcribed at a much lower level (Supplementary Figure 7A), with a maximum of ≈%5 in TH-EPI-Glut. "
                          f"Surprisingly, even if its overall prevalence was much lower then Htr5a, 10 clusters were found to be enriched in Htr5b. "
                          f"This receptor was transcribed at considerable levels only in the 17 MH-LH Glut class (≈50% prevalence). This was caused by high levels of transcription "
                          ...
                          "Some transcription was also evident in the posterior part of the brain, specifically in the inferior olivary complex (IO), a structure strongly linked to cerbellar Purkinje cells {Loyola, 2023 #2914}. "
                          "This transcription was driven by a single supertype, 253 IO Fgl2 Glut.\n",
           ...
           "Htr2": "The Htr2 family is mainly linked to Gq/11 and causes excitation by increasing intracellular Ca2+. Htr2a, famous for being instrumental in mediating the effects of psychedelics {Nichols, 2016 #854},"
                   ...
                   "Colocalization was highest with Htr1f and Htr2c (Figure 5B). Considerable transcription (≈40%) was found in 01 IT-ET Glut, 07 CTX-MGE GABA "
                   "and 16 HY-MM Glut classes (Figure 5C). Interestingly somatotatin (Sst) neuron belonging to 07 CTX-MGE GABA, while exhibiting a relatively low prevalence at the subclass level, contained various clusters "
                   "with >70% prevalence {De Filippo, 2024 #2904}. Htr2a was also prevalent across many other classes across the whole brain. 01 IT-ET Glut exhibited by far the highest absolute number "
                   "of neurons transcribing Htr2a, specifically in subclasses of L5 and CLA, resembling Htr1f (see online visualizer, 'Overview genes by class'). "
                   "Interestingly, L5 extratelencephalic (ET) neurons exhibited considerable "
                   "more Htr1f then Htr2a RNA. "
                   ...
                   f"the Pineal Glut class showed the highest prevalence at {round(joined.groupby('class')['Htr2b'].apply(percentage_above_threshold)['25 Pineal Glut'], 2)}%, an observation never reported previously."
                   ...
                   f"Transcription was broadly distributed across many different classes, also subcortically (Figure 6C). "
                   f"Many classes exhibited a prevalence >60%. "
                   "As usual cortical excitatory neurons exhibited the highest number "
                   f"of cells expressing Htr2c. Some subclasses in OLF, amygdala and retrosplenial cortex (RSP) exhibited >80% prevalence (see online visualizer, 'Overview genes by class'). "
           ...
           "Htr4, Htr6 and Htr7": "These receptor are all connected to Gs {McCorvy, 2015 #2889}, leading to increasing cellular levels of cAMP. "
                                  ...
                                  "Transcription across classes was broadly distributed, with many classes showing a prevalence >40% (Figure 7C). Highest prevalence was found in "
                                  "the 17 MH-LH Glut class, specifically in the Chol releasing neurons of this class located in TH. "
                                  ...
                                  "by subclasses of CA1, CA2, CA3 and subiculum (see online visualizer, 'Overview genes by class').  "
                                  ...
                                  "Dentate gyrus (DG) granule cells (037 DG Glut) were one of the reason of the high prevalence. "
                                  "Excitatory cells of CA2, CA3 and indusium griseum (IG) also transcribed often Htr4 RNA. \n"
                                  ...
                                  f"transcribing the RNA of this gene were excitatory cortical neurons (Supplementry Figure 11C). \n"
                                  
                                  "Conversely, Htr7 was transcibed in >10% of the totality of cells. It reached ≈60% in the TH-EPI Glut group, and considerable amounts (≈40%) in MB, HB and HY groups (Figure 8A). "
                                  "Colocalization was the highest with Htr2c and Htr1f (Figure 8B). transcription was broadly distributed across classes present in HY, MB and TH (Figure 8C). "
                                  "It colocalized the most with Htr2c, "
                                  "tr1f and Htr4. Htr7 was broadly transcribed across classes, especially in subcortical structures. "
                                  "Peak prevalence was found in 10 LSX GABA, 16 MY MM Glut and 18 TH Glut with >60% "
                                  "(Figure 8C). Cortical expression in excitatory neurons is driven primarily by subclasses in CA2 and L2 ENT (see online visualizer, 'Overview genes by class'). "
           ...
           "Htr3": "The Htr3 family is the only ionotropic Htr and it causes direct excitation by allowing the influx of cations. "
                   ...
                   "Htr3a istranscribed almost uniquely in the "
                   ...
                   "with Htr2c and Htr7 (Supplementary Figure 10B). This Htr was mainlytrancribed in OLF, CTXsp, HPF and Isocortex (Supplementary Figure 10D) and "
                   "is most prevalent in the anterior part of the brain, altough, puzzingly, with slightly lower level of transcription (Supplementary Figure 10E-F). "
                   "Htr3b was not included in the MERFISH dataset and no cluster was found to be enriched with this receptor. Htr3b was the leasttranscribed Htr gene in the entire dataset. "
