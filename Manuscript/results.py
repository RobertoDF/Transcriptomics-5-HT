...
results = {"Htrs transcription overview":
               "We analysed the single-cell RNA-seq dataset provided by the Allen Institute {Yao, 2023 #2828} "
                "focusing on the transcription of Htrs genes across approximately 4 million brain cells. The RNA-seq dataset comprehensively encompassed all known 14 Htr subtypes. "
               f"{round(((exp>threshold_expression).sum(axis=1).astype(bool).sum()/exp.shape[0])*100,2)}% of cells transcribed RNA of at least one Htr. To evaluate transcription "
               f"we used the same stringent threshold (log(CPM)>{threshold_expression}) "
               "used by the original authors to determine neurotransmitter release {Yao, 2023 #2828}"
               f"Prevalence of Htrs, the percentage of cells expressing a receptor, across the entire dataset was considerably different ranging from {expression_total.min()}% of {expression_total.idxmin()} "
               ...
               "as neurotransmitter-release {Yao, 2023 #2828}, offer a condensed categorization of cell types (Supplementary Figure 1B, Table 1). "
               f"When looking at the UMAP distributions of individual Htr, considerable differences were also present within each family of receptors (Supplementary Figure 2). "
               f"We analyzed these differences grouping cells by "
               "neurotransmitter, neighborhood or class (Supplementary Figure 1B). The totality of cells were divided in 34 classes (as defined in {Yao, 2023 #2828}). "
               "These categorizations divided cells in a highly skewed fashion (Supplementary Figure 1C). "
               ...
               f"Pattern of Htrs transcription across different neurotransmitter groups exhibited a relatevely high mean Pearson correlation coefficient "
               f"(r={round(corr_by_neurotransmitter.stack().mean(),2)}±{round(corr_by_neurotransmitter.stack().sem(),2)}). "
               f"Sero and cholinergic neurons (Chol) showed the most distinct patterns of transcription with respectively mean r={round(corr_by_neurotransmitter.mean()['Sero'],2)}±"
               ...
               f"To better evaluate the uniqueness of Htrs RNA transcription per group, and account for differences in amplitude, not captured by simple correlation, we employed a Random Forest "
               ...
               f"Chol={round(cm_neurotransmitter['Chol']['Chol'], 2)}%). Cells not transcribing any neurotranmitter, that did not exhibited a low r beforehand, were "
               f", nonetheless, identified even more successfully ({round(cm_neurotransmitter[''][''], 2)}%). Moreover, Noradrenaline (Nora) and glycine (GABA-Glyc) releasing neurons "
               ...
               f"NN-IMN-GC includes all the cells not releasing any neurotransmitter, their classification was therefore expectedly influenced by absence of any Htr. On the other hand, "
               ...
               f"Across classes, differences in Htrs transcription were even more striking (Figure 1D). {report_class.loc['recall'][report_class.loc['recall']>.4].shape[0]} groups could be identified "
               ...
               f"were identified by absence of any Htr transcription. The exclusive use of Htrs transcription pattern reached a "
               f"impressive {round(accuracy_class,2)}% accuracy in decoding classes. \n"
               ...
               f"(Supplementary Figure 4A). For example, Pallium-Glut exhibited a unique negative correlation between Htr4-Htr2a not visible "
               f"from the analysis of the entire dataset. Of note, TH-EPI-Glut showed the "
               f"highest absolute correlation across all neighborhoods with r={correlation_TH_EPI.max().max()} between Htr5b-Htr4 and a unique negative correlation between Htr4-Htr7. "
               "To explore the underlying causes of the correlations we analyzed colocalization between Htrs. "
               f"Across the entire dataset we observed that the most transcribed genes, Htr1f and Htr2c, were regularly transcribed whenever the RNA of any other Htr was detected (Figure 1F). "
           ...
           "Htr1 & Htr5": u"Receptors belonging to these two families have an inhibitory effect on the host cell, they are coupled to G\u1D62 and cause a downstream decrease of cAMP "
                          ...
                          "and showed a good correspondance in absolute values, this was the case for the majority of others Htrs. "
                          "Highest transcription was found in Sero neurons of the mid- and hindbrain (class 22 MB-HB Sero, Figure 2C), nonetheless, cortical excitatory neurons (01 IT-ET Glut), "
                          "like in the majority of Htrs, "
                          "contained the highest absolute number of cells transcribing the receptor. Subclasses located in the hippocampus (HPF) "
                          "contained most of the cells transcribing Htr1a (see online visualizer, 'Overview genes by class'). "
                          "To pinpoint the spatial location, we first identified in the RNA-seq datasetbthe clusters highly enriched with Htr1a RNA with "
                          ...
                          f"a cluster must express the receptor. "
                          ...
                          "two structures linked to generation of theta waves {Winson, 1978 #2908} and contaiing Chol neurons, contributed substantially to the transcription in PAL. "
                          "Notably, "
                          ...
                          f"Also in this case, 01 IT-ET Glut exhibited the highest absolute number of Htr1b expressing cells, specifically a subclass "
                          ...
                          f"Htr1f, surprisingly, showed the highest levels of transcription of all Htrs in the RNA-seq dataset. "
                          ...
                          f"(see online visualizer, 'Overview genes by brain structure'). "
                          f"Isocortex and HPF "
                          ...
                          f"although only at lower levels compared to other Htrs (Supplementary Figure 6C). Only one cluster was considered enriched with Htr5a in the entire "
                          ...
                          f"Htr5b was transcribed at a much lower level across neighborhoods (Supplementary Figure 7A), with a maximum of ≈%5 in TH-EPI-Glut. "
           ...
           "Htr2": "The Htr2 family is mainly linked to Gq/11 and causes depolarization by increasing intracellular Ca2+. Htr2a, instrumental in mediating the effects of psychedelics {Nichols, 2016 #854},"
                   ...
                   f"of cells expressing Htr2c. Some subclasses in OLF, amygdala and retrosplenial cortex (RSP) "
                   f"exhibited >80% prevalence (see online visualizer, 'Overview genes by class'). "
                   ...
                   f"curiosly the area with lowest transcription of Htr1f, otherwise highly prevalent in all other cortical regions. Htr2a was also minimally expressed in this specific area. "
           ...
           "Htr4, Htr6 and Htr7": "These receptor are all connected to Gs {McCorvy, 2015 #2889}, leading to increasing cellular levels of cAMP. "
                                  ...
                                  "Dentate gyrus (DG) granule cells (037 DG Glut) were one of the reason of the high prevalence in HPF. "
                                  "Excitatory cells of CA2, CA3 and indusium griseum (IG) also transcribed Htr4 RNA (see online visualizer, 'Overview genes by brain structure'). \n"
                                  ...
                                  "Conversely, Htr7 was transcibed in >10% of the totality of cells. "
