...
results = {"Transcriptomic overview of 5-HT receptors landscape":
               ...
               f"When looking at the UMAP distributions for individual Htr, considerable differences were present also within family (Supplementary Figure 3). "
               f"We analyzed this differences more in detail by grouping cells by "
               f"neurotransmitter, neighborhoods or class. These categorizarions divided cells in a higlhy skewed manner (Supplementary Figure 4), "
               ...
               f"r={round(corr_by_neurotransmitter.mean()['Chol'], 2)}{round(corr_by_neurotransmitter.sem()['Chol'],2)} (Supplementary Figure 5). "
               f"To evaluate the uniqueness of Htrs per class we employed a Random Forest Classifier aiming at decoding the grouping variable from the Htrs expression. "
               f"Overall accuracy of the model was {round(accuracy_neurotransmitter,2)}%. "
               f"Reflecting the correlation analysis, Sero and Chol groups were among the groups with higher true positive (TP) rate (Sero={round(cm_neurotransmitter['Sero']['Sero'], 2)}%, "
               f"Chol={round(cm_neurotransmitter['Chol']['Chol'], 2)}%). Cells expressing noradrenaline (Nora), histamine (Hist) and no Htr were also "
               f"identified more than 50% of the times (Nora={round(cm_neurotransmitter['Nora']['Nora'], 2)}%, "
               f"Hist={round(cm_neurotransmitter['Hist']['Hist'], 2)}% and no-Htr={round(cm_neurotransmitter[''][''], 2)}%). To understand the contribution of each Htr in each prediction "
               f"we calculated the mean absolute SHAP values for each receptor and neurotransmitter. Here we can see, for example, that the Sero group is determined mainly by expression of Htr1a. "
               f"When looking at different neighborhoods the accuracy of the model was {round(accuracy_neighborood,2)}%. "
               f"The model could differentiate best the NN-IMN-GC and TH-EPI-Glut groups (Supplementary Figure 6). "
               f"NN-IMN-GC includes all the cells not releasing any neurotransmitter, predictions belonging to this group were strongly influenced by absence of Htr2c. On the other hand, "
               f"TH-EPI-Glut cells were characterized by expression of Htr7. Across classes, differences in Htrs expression were more clear (Figure 1D). Various groups could be identified "
               f"with a TP rate > 50%: 04 DG-IMN Glut, 09 CNU-LGE GABA and 34 Immune. 04 DG-IMN Glut and 09 CNU-LGE GABA were characterized, respectively, by high expression of Htr4 and Htr1b. "
               f"34 Immune cells showed virtually absence of any Htrs expression. "
               f""
               f""
               f""
