...
methods = {"Jupyter notebooks structure": "The entire analysis is contained in 2 jupyter notebooks hosted on Github at https://github.com/RobertoDF/Transcriptomics-5-HT. "
            ...
            "Data preparation":             "We loaded the metadata and the precomputed expression matrices ('exp' dataframe) for the scRNA-seq dataset relative to all Htr genes "
                                                      ...
                                                      "this is necessary because some cells are assigned to multiple 'cluster_group_name' and therefore cause the duplication of the dataframe´s rows. "
            ...
            "Overview figure": "This figure relies uniquely on the scRNA-seq dataset. "
                                                     ...
                                                     "\nIn panel B we used the precomputed UMAP coordinates available in the 'joined' dataframe and plotted on the color axis information about the "
                                                      ...
                                                      "In panel C we plotted the percentage of cells transcribing each Htr grouped by neurotransmitter release. "
                                                        "We take advantage of the Pandas 'Group by' "
                                                     ...
                                                     "This function uses a boolean version of the 'joined' dataframe created using "
                                                      ...
                                                      "In this particular case, the features for classification were defined as the boolean expression of the various 5-HT receptor genes, and the target variable was "
                                                      "the neurotransmitter type. A Random Forest classifier was initialized with 200 estimators, a maximum depth of 10, balanced class weights, and parallel processing across 20 jobs. "
                                                        "Linear models such as 'LogisticRegression' and 'LinearDiscriminantAnalysis' were found to underperfom the Random Forest classifier (see 'Test linear models' in Figure_1.ipynb). "
                               "Using Stratified K-Fold cross-validation with 5 ('n_splits' set in Utils.Settings.py) folds, balanced accuracy scores were computed, and the mean accuracy was reported. The model "
                                                      ...
                                                      "dataset. Predictions were generated with cross-validation. "
                               ...
                               " Additionally, a comprehensive classification report was generated, providing metrics such as "
                                                      ...
                                                      " classification tasks. The evaluation of the model's performance was performed using scikit-learn's 'balanced_accuracy_score', 'classification_report', "
                                                     ...
                                                     "In panel D we plotted the percentage of cells transcribing each Htr grouped by class label, additional plots related to classification accuracy were computed following the instructions of the previous panel and are available as supplementary figure.  "
            ...
            "Receptor figure":  "This figure relies on both the scRNA-seq and MERFISH datasets. "
                                                             ...
                                                             "We excluded from the analysis the 'NN-IMN-GC' neighborhood because of consistently low transcription across all Htr genes. For the visualization "
                                                              "of gene expression patterns across different 'neighborhoods', we the Seaborn 'pointplot' function to illustrate the transcription levels of a given gene across various groups. The 'violinplot' "
                                                              ...
                                                              "First, we calculated in the scRNAseq the percentage of cells within each cluster expressing the target gene above a defined threshold ('threshold_expression'), grouping the data by cluster. "
                                                              "This allowed us to identify clusters with high gene expression levels (>70%, 'threshold_enriched_clusters' in Utils.Settings.py) in the scRNA-seq. "
                                                                "Next, we focused on clusters with significant gene expression, "
                                                              ...
                                                              "pie chart highlighted the contribution of these enriched clusters to the overall expression of the target gene. This pie chart shows that in some cases only a minority of cells "
                                                              "transcribing a selected gene belongs to enriched clusters, consequetially the majority of cells will be ignored. "
                                                              "To address this problem we include in the interactive visualizer ('Overview genes by brain structure') "
